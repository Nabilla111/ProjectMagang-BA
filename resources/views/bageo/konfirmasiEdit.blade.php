...
<form action="{{ route('bageo.updateStep5', ['id' => $bageo->id]) }}" method="POST">
    @csrf
    <div class="container">
        ...
        <ul>
            <li>Tanda Tangan:
                @php
                    $ttd = \App\Models\TtdKabid::find($bageoDataStep1['id_ttd'] ?? null);
                @endphp
                {{ $ttd ? $ttd->nama : '-' }}
            </li>
            <li>Jenis BA: {{ $bageoDataStep1['jenis_ba'] ?? '-' }}</li>
            <li>Instansi: {{ $bageoDataStep1['instansi'] ?? '-' }}</li>
            <li>Tanggal Berita Acara: {{ isset($bageoDataStep1['tanggal_bageo']) ? \Carbon\Carbon::parse($bageoDataStep1['tanggal_bageo'])->translatedFormat('d F Y') : '-' }}</li>
        ...
        <ul>
            ...
            <li>Waktu Unggah: {{ isset($bageoDataStep2['waktu_unggah']) ? \Carbon\Carbon::parse($bageoDataStep2['waktu_unggah'])->translatedFormat('d F Y') : '-' }}</li>
        ...
        <ul>
            ...
            <li>Catatan: {{ $metadataValidatedData['catatan'] ?? '-' }}</li>
            <li>Saran: {{ $metadataValidatedData['saran'] ?? '-' }}</li>
